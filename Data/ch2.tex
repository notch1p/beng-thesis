...
{\setlength{\parskip}{-1em}
  ...
  允许其存在，即可构造不动点组合子（fixpoint combinator），其中又以 \texttt{Y} 组合子最为著名（其中一种实现）：
  \begin{equation}
...
let compose f g x = f (g x) ;; infixr 90 "." => compose -- 函数复合
...
{\setlength{\parskip}{-1em}\subparagraph*{全局定义} 与本地定义几乎一致，但如其名，仅限于顶层（toplevel）。
  顶层的 Let(rec)-表达式不必再拥有其程序体（即没有 \mintinline{ocaml}{in ..} 从句），退化为一个独立的定义。多个
...
仅在此处做一小区分：\underline{声明不包含右手侧}。在上面给出的样例之外，\Tigris 还支持 Haskell 风格的由 Let 直接绑定的运算符定义语法：
\begin{minted}[frame=single]{ocaml}
...
扩展的 Let-绑定能够在其左手侧接受二元运算符的应用的形式，而算符两侧则可以接受模式。
通过这种形式定义的运算符不一定需要运算符声明，默认优先级 50，左结合。
这一模式除了形式上更加自然，最重要的优势在于其搭配 Let-表达式，
可以允许用户局部地、快捷地使用暂时定义的运算符，而不必在顶层写大量的运算符声明。
从上面的例子还可知道这种 Let-绑定亦支持递归定义。在目标代码生成中，运算符 \texttt{op} 有形式 \texttt{«op»}.
