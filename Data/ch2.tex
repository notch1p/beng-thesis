...
{\setlength{\parskip}{-1em}
  ...
  \subparagraph{Skolem TV}\footnote{纪念挪威数学家 Thoralf Skolem，主要研究数理逻辑与集合论。} 亦称 skolems，记作 \texttt{?sk.a} 其中 \texttt{a} 指代
...
\end{minted}
  ...
  $impl$ 对应元语言（Lean）中经 $\eta$-变换的 \texttt{<expr>}，类型 $\mathsf{Expr}\to\mathsf{Expr}\to\mathsf{Expr}$\footnote{
    熟悉代数的读者马上知道，这里的 $impl$（譬如 \texttt{App : Expr -> Expr -> Expr} 结点）与对象语言（\Tigris）中的二元运算符、
    函数应用语法\emph{同态}（即两者在这一映射下结构不变），由同态射 $\mathsf{chainl/r_p}(impl)$ 给定。
...
具有副作用的函数，就可以利用 FFI. 我们认为这是一个高级功能，参见小节 \ref{sect:ffi} 中更细致的描述。
