...
\section{词法惯例}
在下文我们用一套修改版的 BNF（Backus-Naur Normal Form，勿和 ABNF 混淆）来表示表层语法。修改过的 BNF 可以在其
...
用大步操作语义（big-step operational semantics）\upcite{kahn1987natural,plotkin1981structural}表示为
...
\section{类型表达式}
\begin{definition}[类型表达式]\label{def:tyexp} 由以下 BNF 给出
\begin{minted}{bnf}
...
一个记作 \((\alpha : n)\) 的 TV 有类型 \(\mathbf{Type}\overbrace{\to\cdots\to}^{n\text{ 个 }\to}\mathbf{Type}\)，否则默认为 $\mathbf{Type}$.
...
类型表达式的合法性检查（validity checking）基本在句法分析阶段完成——也就是说，
若存在某一 TV 是自由变量（free variable），
编译器会抛出句法分析错误。在用户接触到的 TV 外，实现中编译器还存在以下两种特殊含义的 TV：

{\setlength{\parskip}{-1em}
  \subparagraph{元变量（metavariables）} 亦称 MV，记作 \texttt{?m.N} 其中 \texttt{N} 是计数器的数字。元变量指代某一未知类型，且必须被
  某一类型所实例化（instantiation）\footnote{
    “实例化”一词在本文中经常使用，但其具体含义没有清晰的定义：有时用这词指代特殊化（specialization），
    指 TV 被\emph{实际}（concrete）类型所实例化；有时只用其指代 TV 的替换（substitution）操作。
  }
  、或在离开作用域时被 HM 系统的自动泛化机制所泛化。无论如何，MV 不应在最终的模式（scheme）\footnote{
    与“实例化”一词相同，“模式”一词，既指代模式匹配中的模式（pattern），亦指代公理模式（此处为类型模式）中的模式（scheme）。
    虽然容易造成歧义，但两者相隔甚远，读者应当能自行区分其具体意思。
  }
  中出现。

  \subparagraph{Skolem TV} 亦称 skolems，记作 \texttt{?sk.a} 其中 \texttt{a} 指代
  原始 TV. Skolems 由 skolemization 过程产生，这些 TV 实际上由类型构造子编码\footnote{在实现中，我们用 \texttt{TCon} 编码，避免错误的合一化。}，并且\emph{绝对不}应被实例化。这使得类型检查器能够查出更多的类型错误，考虑
}
\begin{minted}[escapeinside=\%\%,frame=single]{lean4}
instance : Functor Option =
  {map | f, None => None -- %$\forall\alpha,\beta.\ (\alpha\to\beta)\to\mathsf{Option}\ \alpha\to\mathsf{Option}\ \beta$%
       | f, Some a => Some a} -- Can't unify type ?sk.a with ?sk.b.
\end{minted}
...
{\setlength{\parskip}{-1em}
  ...
  允许其存在，即可构造不动点组合子（fixpoint combinator），其中又以 $\mathsf{Y}$ 组合子最为著名（其中一种实现）：
  \begin{equation}
  ...
  整数，也可以是字符串等等。更危险的问题来源于后文 Tigrisd 定理证明器的理论基础 Church-Howard 同构（isomorphism）\footnote{对直觉主义逻辑（类型论）更准确的说法是 Brouwer-Heyting-Kolmogorov（BHK）解读。}，
...
let f1 : ∀ α, List α → α × α = Λ α. fun ?x₀ : List α =>
  match ?x₀ with | Cons x (Cons y _) => (x , y)
let f2 : ∀ α, List (List α) → α × List α = Λ α. fun ?x₀ : List (List α) =>
  match ?x₀ with | Cons (Cons x y) _ => (x , y)
\end{minted}
...
{\setlength{\parskip}{-1em}
  ...
  编译器会抛出元数不匹配错误。
  \paragraph*{元组模式} 有格式 $(p_1,\dots, p_m)$，因为其可看作由\emph{二元逗号构造子}$(-,-)$构成的构造模式，
  而该二元构造子又可看作是右结合的中缀运算符，因而元组（模式）都是右结合的。
...
\begin{table}[H]
  \begin{tabularx}{\linewidth}{@{}lX@{}}
    ...
    \(v, v', v_i\)               & 表示值；          \\
    \(a, b, c, d, x, y, z, w, f, g, h\) & 表示标识符；$f,g,h$ 在语义上指函数；        \\
    \(\sem{}\)            & 是黑板风格字体，在元语言中普通分界符用光时用作分界符； \\
    希腊字母   & 表示类型。其上一横杠 \(\bar{\alpha}\) 指一序列，有隐式编号 \(1,\dots,i\). \\ \bottomrule
  \end{tabularx}
\end{table}
...
{\setlength{\parskip}{-1em}\subparagraph*{函数简写语法} 指的是受 Lean 启发的括号语法糖。在括号内，所有的下划线（\texttt{\_}）或
...
{\setlength{\parskip}{-1em}
  ...
  }在实现上会直接被翻译为基本语法构造 \mintinline{ocaml}|match .. with|.
  上述的最简 Let-绑定形式可以用于直接绑定（或者在传统意义上，定义）函数形式的值，但 \Tigris 也提供了语法糖用于定义函数。
  在一个 Let-表达式中，可以通过 \(\mathbf{let}\ f\ p_1\cdots p_n = e\)、模式矩阵（pattern matrices）式的 \(\mathbf{let}\ f\mid p_{i,j}\Rightarrow e\) ，或这两种
  形式的混合 \(\mathbf{let}\ f\ \overline{p}\;\overline{\mid \overline{p}\Rightarrow e}\)\footnote{
    回忆：在某一对象上加横线表示\emph{一系列}这样的对象。
  }
定义函数，例如：}
\begin{minted}[ignorelexererrors=true,frame=single]{haskell}
let prodSum (x, y) z w = x + y + z + w
and (x, y) = (1, 2) in prodSum (x, y) 3 4

let (true) = false -- true 不等于（无法匹配）false

infixr 90 "." ;; let f . g = fun x => f $ g $ x
...
\end{Verbatim}
这说明类型检查器能够检查某个模式是否匹配全面（exhaustive）。显然，对一个匹配不全面的模式，
如果右手侧出现了无法匹配的值而匹配失败，程序在该点的行为即是未定义的。
实现上，如果运行期匹配失败，将通过 Common Lisp 的条件系统
（condition system）抛出一个底层错误——无法恢复（nonrecoverable）的 \texttt{MATCH-FAILURE}
错误，由 LISP debugger 中止正在运行的程序，等待用户输入。

{\setlength{\parskip}{-1em}
  \subparagraph{本地递归定义} 通过 Letrec 语法构造引入（模式矩阵式的语法仍然可用）：
  \begin{equation}
...
{\setlength{\parskip}{-1em}\subparagraph*{全局定义} 与本地定义几乎一致，但如其名，仅限于顶层（toplevel）。
  顶层的 Let(rec)-表达式不必再拥有其程序体（即没有 \mintinline{ocaml}{in ..} 从句），退化为一个独立的定义。多个顶层定义可以有选择的用 \texttt{;;} 作分隔，提高可读性。
...
即能够推断\emph{最泛化}的类型的能力。换句话说，从 Python/Ruby/Perl 或
任何不需类型标注的动态语言迁移过来的用户都能保持不写类型标注的习惯（仅语法上），
同时享受静态类型带来的安全保障。
...
实现多态不同，\Tigris 充分利用自动泛化机制，保证“\underline{能泛化的程序\emph{一定}会被自动泛化且无需类型标注}”这一性质。

搭配这两个不同于一般语言的特性，与常见的工业语言相比，用户能获得异常好的可读性。
图 \ref{code:ts-tig} 同 Typescript（渐进类型系统，gradual typing）\footnote{
...
将行向量表达式 $e_1,\dots, e_m$ 与模式矩阵按并行求值顺序相匹配\footnote{
  Cf. 积模式则是按前序遍历顺序匹配的。
}，并返回第一个成功的分支的值。类型检查器通过一个高效的，基于矩阵模型的算法，对模式矩阵
开展完全、冗余（redundancy）性检查，其中前者用于检查某一模式矩阵是否能够匹配所有情况，
后者用于检查这一矩阵中的任意两个行向量是否重复，若重复则向用户报警，并立即删除多余行向量。
直观上这一行为是正确的：若出现多个重复的行向量，则依先来先得原则取第一个分支；在运行期，
若所有分支匹配失败，抛出运行时错误 \texttt{MATCH-FAILURE}.
\begin{minted}[frame=single]{haskell}
let (1, 2) = (1, 2) -- possible cases such as [(_, _)] are ignored
let f | (x, _) => x | (_, y) => y -- redundant alternatives at 3) (_, y)
\end{minted}
读者需要注意，完全性检查与运行时的模式匹配毫不相干：考虑上方范例一，根据右手侧的值，我们
很清楚左侧模式一定匹配成功，但同样的结论并不适用于更复杂的右手侧。模式匹配仅在运行时执行，
编译期的完全性检查仅根据左侧模式及右侧表达式的类型作出完全性判断——具体行为与代码由模式匹配编译模块决定和生成。

模式匹配在语义和实现上可以看作是命令式语言中的 \texttt{switch} 语法结构的超集，
它允许用户声明式（declarative）地解构某数据结构，是函数式编程的核心功能之一。
...
显式地写出各种投影（例如数组访问）操作，而仅仅是告诉编译器：“满足这一模式的数据结构才能匹配”——
由编译器自行生成访问数据结构的代码。
...
\begin{minted}{bnf}
<infixl-decl> ::= infixl  ":"? <op> <prec> (ARROW <expr>)?
<infixr-decl> ::= infixr  ":"? <op> <prec> (ARROW <expr>)?
<prefix-decl> ::= prefix  ":"? <op> <prec> (ARROW <expr>)?
<postfix-decl>::= postfix ":"? <op> <prec> (ARROW <expr>)?
\end{minted}
  其中 \texttt{<op>} 表示运算符对应的字符串，且为 \texttt{<strlit>} 的子集；
  以供 \texttt{chainl/chainr} 组合子使用。
  $impl$ 对应元语言（Lean）中经 $\eta$-变换的 \texttt{<expr>}，类型 $\mathsf{Expr}\to\mathsf{Expr}\to\mathsf{Expr}$\footnote{
    熟悉代数的读者马上知道，$impl$（譬如 \texttt{App : Expr -> Expr -> Expr} 结点）与对象语言 \Tigris 中的二元运算符、
    函数应用语法\emph{同态}（homomorphic，即两者在这一映射下结构不变），由同态射 $\mathsf{chainl/r_p}(impl)$ 给定。
    换句话说，\underline{\Tigris 中的所有二元运算符或函数应用，在实现中都有对应的 $impl$ 以替换之}。
  }.
  运算符不仅出现在表达式中，若其绑定构造子则亦可在模式中出现，例如前文的 \texttt{(::)}.
\end{definition}
...
\end{definition}
三种类型（前/中/后缀）的运算符的优先级（再加上函数应用）与模式中的相同，见表 \ref{tab:patoperator-prec}.
虽然存在无穷多的候选字符串可供用户选择用来声明运算符，但我们也用同种方式
内置了一些常见的运算符（其定义仍然可被覆盖）\footnote{
  虽然在实际应用中，更常见的思路是将运算符绑定到类型类的方法上以方便调用接口方法，但此处我们
  去繁从简；此外，目前而言我们的系统中也确实不包含序章定义（prelude，语言最基本定义的源文件）。
}\,\footnote{
...
\begin{table}
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \texttt{\$}、\texttt{@@}       & 1          & infixr（中缀右结合）& 函数应用             \\ \bottomrule
  \end{tabular}
\end{table}
...
不仅如此，对于前缀/后缀算符而言，我们可以把一个字符串同时声明为这两种类型的运算符（但目前无法将同一字符串同时声明为前缀/中缀或后缀/中缀），
例如：
\begin{minted}[frame=single]{lean4}
postfix 65 "!" => fact ;; prefix 65 "!" => not ;; (50!, !true)
\end{minted}

\subparagraph{由运算符扩展的 Let-绑定}
根据运算符定义的语法，我们会发现其右手侧并非必须：虽然本文将\emph{声明}、\emph{定义}混用，
仅在此处做一小区分：\underline{声明不包含右手侧}，如 \mintinline{haskell}|infixl 70 " #+# "|.
在这一区分之上，\Tigris 支持 Haskell 风格的由 Let 直接绑定的运算符定义语法：
\begin{minted}[frame=single]{ocaml}
...
扩展的 Let(rec)-绑定能够在其左手侧接受二元运算符的应用形式，而算符两侧则可以接受模式。
当这种形式的 Let 产生歧义，即如 \mintinline{ocaml}|let x :: xs = ..| 可以同时解糖为
模式匹配或运算符定义，则模式匹配优先。
通过这种形式定义的运算符不一定需要声明，默认优先级 50，左结合。
这种定义除了形式上更加自然，最重要的优势在于其作为表达式时
允许用户局部、快捷地暂时定义、使用运算符，而不必在顶层写大量的运算符声明。
在目标代码生成中，运算符 \texttt{op} 有形式 \texttt{«op»}.
...
虽然 \Tigris 是一个纯函数式语言，但与 Haskell 或 Lean 不同，其不强求用户——若直接引入
具有副作用的函数，就可以利用 FFI. 这是一个高级功能，参见小节 \ref{sect:ffi} 中更细致的描述。
...
下文做一简单理论介绍，亦为后文 \Tigrisd 叙述先验知识。
...
而称其为代数数据类型，是因为这一概念的理论基础来源于范畴论的自函子\footnote{
  ...
  但在 Lean 这样具有非累加的（noncumulative，与 Coq 不同）
...
实际上，各大定理证明器亦确实会为所有归纳类型（在证明器的上下文中是 inductive/indexed family）
导出消去子，作为最基础的计算规则。具体地说，任意消去子的参数都应该包含
\vspace{-.6em}
\begin{enumerate}\itemsep-.2em
  \item\emph{动机}（motive），即消去子的目标类型（想要证明的命题）。从操作语义的角度考虑，这是一个续点（continuation），
    会被定理证明器的内核应用;
...
\end{enumerate}
\vspace{-.6em}
但这些内容已超出本文范围，读者可了解递归论（计算理论）、证明论和范畴论。
归纳类型具有较强的理论特性，根植于数学，但这并不妨碍其在工程上具有的重大意义，一些现代的主流语言，如
Rust、Swift 亦已支持这一特性，且广泛运用于编程实践中。在 \Tigris 中，归纳类型定义具有以下格式：
\vspace{-1em}
\begin{equation}
...
以下式子给定一个值构造子的类型（其中小写的 $c_i$ 表示 $C_i$ 的元数）\footnote{
...
\begin{minted}{bnf}
<class-decl>  ::= class <type-ctor> <type-binder> = <type-record>
\end{minted}
\end{definition}
...
传递给调用对应方法的函数实现方法分派；具有格式：
\begin{equation}
  \mathbf{class}\ C\ \bar{\alpha} = \{\overline{f}\}\text{ where }   f_i : \forall \bar{\alpha}, \bar{\beta}.\, \sigma_i
\end{equation}
...
\(\forall a\ b\ [\mathsf{HEq}\ a\ b].\, a\to b\to \mathsf{Bool}\)；
而某个调用了上面所有方法且没有提供任何具体类型去精练（refine）类的程序 $p : \tau$，具有类型：
$
\forall a\;b\;f\ [\mathsf{Functor}\ f, \mathsf{HEq}\ a\ b].\,\tau
$.
...
表达异性等号（heterogenous equality）的交换律（commutativity），并通过 $\mathsf{Eq}\ a\to \mathsf{HEq}\ a\ a$ 表达\underline{二元关系的同性等号是异性等号的子集}这一事实。
...
加上类型标注如 \texttt{f' : Int} 才能令编译器找到第二行定义的实例。
...
在实践中，组合句法分析类似递归下降分析器（recursive descent parser），或者说类似手写的分析器。对应的文法与
解析表达文法（parsing expression grammar，PEG）\upcite{10.1145/964001.964011}相似：两者遇到歧义语句时总是选择第一条匹配的生成规则（production rule）。
但目前而言，不同于 Lean 的实现，\Tigris 的句法分析模块还不是 Packrat 分析器（PEG 的其中一种），后者能够在线性时间内完成分析。
此外，因为使用单子设计模式，我们的实现仍然存在大量的回溯（backtracking）操作。句法分析单子 \texttt{TParser} 类型为
...
相比利用栈空间的 \texttt{StateT} 更高效。上述定义中的单子转换栈（monad transformer stack）还附带了其余两个
转换器 \texttt{SimpleParserT}、\texttt{StateRefT}，
这使得我们可以在 \texttt{ST} 中利用类似 \texttt{State} 的接口方法以流畅地编写代码，同
时 \texttt{Substring} 的使用允许通过一对指针
在同一字符串上做计算（句法分析）而不产生任何中间字符串数据，保证内存使用的高效性。

关于组合子本身是如何实现的，我们首先考虑任意两个句法分析器 $P, P'$，给定输入 $j$（例如字符串）
则可以定义 \emph{alternative} 组合子 \(P\oplus P'\ (j)=P\ j\cup P'\ j\)；
\emph{sequence} 组合子 \((P\circledast P')\ (j)=\bigcup \{P'\ k \mid k\in P\ j\}\)
...
关于不同类型论的表达力，我们可以在 Barendregt 提出的 Lambda Cube\upcite{Barendregt_1991} 中清晰比较。
从左下角到右上角，这一立方体代表的类型论的表达力逐渐增强，更具体的说，
箭头 $\to$ 表示那些类型可以依赖值的类型论，属于依值类型\footnote{即依赖类型，此处为和下文区分做调整。}；
箭头 $\uparrow$ 表示项可以依赖类型，即具备多态；
...
\begin{wrapfigure}{l}{6cm}\centering
  ...
  \caption{Lambda 方块\upcite{lambdacube}}\label{fig:lambda-cube}
\end{wrapfigure}
...
同时，为了方便对编译器做 debug，我们利用与 Wadler 相似的 pretty-printing 算法\upcite{hughes1995design}打印 \texttt{FExpr} AST，
...
\section{本章小结}
本章以语言报告、教程的风格，搭配范例、反例介绍了 \Tigris 语言的各方面功能与规范，
并在最后主要谈到了词、句法分析的实现细节。我们认为在探究某一语言的内部结构、背后理论时，
最基本且必要的先验知识应当是作为一个用户可以使用熟练，而在短时间内使得读者能够尽快熟悉起语言的方法莫过于大量的实例，
因而，读者应当重点思考本章的每一个实例。
这一章为后文集中的理论叙述打下重要基础。