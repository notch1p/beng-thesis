...
\begin{itemize}\itemsep-.5em
  ...
  \item\textbf{不动点组合子应用（Fix）}\quad 即 \(\mathbf{fix}\ \lambda\ f.\ e\) 具有 \(\rpnt{e}\) 的类型；\footnote{
      ...
      但最后考虑实际效用便放弃维护。不动点组合子的语法是 \mintinline{ocaml}|rec expr|，未被删除，但不一定正确编译/解释。
    }
\end{itemize}
\vspace{-1.4em}
...
\begin{definition}[Skolemization]
  ...
  \begin{equation}
    \skolem(\forall \overline{\rpnt{tv}}.\, \bar{\pi} \Rightarrow \rpnt{ty}) =
    [\overline{\rpnt{tv}} \mapsto \overline{\mathsf{sk}_\rpnt{tv}}](\rpnt{ty})
  \end{equation}
  其中每一 \(\mathsf{sk}_\rpnt{tv}\) 都是崭新的 skolem 常量（通过0-元类型构造子实现，
  在实现中有形式 \texttt{?sk.\(\rpnt{tv}\)}）。
  “刚性”一词很好地表达了 skolem 的关键性质：
  \underline{\(\mathsf{sk}_\rpnt{tv} \sim \rpnt{ty}\) 正确，当且仅当 \(\rpnt{ty} = \mathsf{sk}_\rpnt{tv}\).}
\end{definition}
...
\begin{definition}[刚性变量集合/栈 rigid set/stack]
  ...
  \begin{equation}
    \begin{aligned}
      \pushRigid(\bar{\rpnt{tv}}) &: \quad \rigid := \rigid \cup \overline{\rpnt{tv}};\quad
      \mathit{stack} := \overline{\rpnt{tv}} ::  \mathit{stack} \\[-5pt]
  ...
  \begin{property}[Invariant]
    ...
    \(\rpnt{tv} \mapsto \rpnt{ty}\) 满足 \(\rpnt{tv} \in \rigid\) 和 \(\rpnt{ty} \neq \rpnt{tv}\) 都将被拒绝。
  \end{property}
\end{definition}
...
\begin{remark}[求解刚性变量]
  ...
  \begin{mathpar}
    ...
    {\rpnt{tv} \in \rigid \\
    \rpnt{ty} \neq \rpnt{tv}}
    {\solvesf(\rpnt{tv} \sim \rpnt{ty}) = \mathbf{error}}

    \inferrule*[right=Solve-Rigid-Ok]
    {\rpnt{tv} \in \rigid}
    {\solvesf(\rpnt{tv} \sim \rpnt{tv}) = \varnothing}
  \end{mathpar}
\end{remark}
...
此外，对 HM 系统添加的扩展也日渐影响着我们的类型系统的合理性。下面，我们论述本系统是通过何种机制保证合理性的。
...
\begin{algorithm}\caption{Instance Resolution}
  \begin{algorithmic}[1]
...
\begin{property}
  ...
  {\setlength{\parskip}{-1em}
    ...
    检测直接的环（自环），但不一定保证停机，尤其是针对一些边界情况。
    ...
    也不会导致其被优先选择，而是出现歧义错误，尽管优先选择它在工程意义上看起来可能更合理。若允许多个匹配实例的存在，
...
但不同于以往，$\mathsf{F}$ 基于 \texttt{EStateM}，一个自带错误处理与（用纯函数模拟的）可变状态的单子，
...
\begin{remark}[System F IR 的实用性与设计上的反思]
  ...
  变量的\emph{形状}来产生代码，但这并不可靠，且存在一些笔者临时修补出现的新错误。
  不仅如此，若后期需要添加静态类型语言后端，比如 C，这一 IR 就几乎不可用。
