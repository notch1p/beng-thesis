...
\begin{enumerate}
  ...
  \item {\bf $\eta$-expansion} 不仅仅指 \lambdacalc 中的计算法则，在此处更指用于解决降级的函数应用（\texttt{App}）分支并未正确
    处理的边界情况。考虑应用通过模式匹配投影产生的函数，如（\texttt{Boxed} 是一构造子）
...
in unwrap boxed 20 20
    ...
    产生\emph{语义不一致}问题——且我们无法避免其发生。因此，与其无条件地做 $\eta$-expansion，在具体实现中我们只对
  ...
  \item {\bf 栈底层降级工作} CPS 风格的递归代码在降级过程中是经常使用的，而递归的首要工作就是明确基准情况（base case）——续点栈（continuation stack）底层的
    ...
    前者定义为：
    \begin{Verbatim}[commandchars=\\\{\},codes={\catcode`$=3},numbers=left,baselinestretch=1]
...
\subparagraph{尾调用（tailcall）优化} 是函数式语言中几乎必备的一个优化。使用任一函数式语言，或阅读过 \emph{SICP}\footnote{
...
\begin{figure}[htb]
  ...
  \end{tikzcd}\]
  \caption{CPS 化在元语言与对象语言间的同像性}\label{fig:cps-homoicon}
\end{figure}
...
我们下面给出 CPS 变换的指称语义\footnote{
  ...
  具体实现。
}，与 CPS IR 自身的指称语义。如同第二部分介绍类型系统一样，首先我们给出 \Lambdair、CPS IR 的形式文法。
...
\begin{align*}
  ...
  \tau \in \mathsf{Tail} &::=\mathbf{ret}\, x \mid f(a)^{\mathsf{T}}\mid \mathbf{if}\, c\, \mathbf{then}\, e_1\, \mathbf{else}\, e_2 \mid \fail \\
...
\begin{equation}\label{eq:lambda-cps-formal}
  \begin{aligned}
    ...
    \tau_c \in \mathsf{CTail} &::=f(p, k) \mid k(v) \mid \halt(v) \mid \ite(c, e_1, e_2) \mid \fail \\
    &\mid \switch^{\Const}(x, \overline{k_i \to e_i}, e_?) \\
    &\mid \switch^{\ctor}(x, \overline{(t_i, n_i) \to e_i}, e_?) \\[4pt]
...
\begin{align*}
  ...
  \end{cases}\\
  \text{where}\quad\Env' &= \{p_f \mapsto \Env(p),\, k_f \mapsto \Env(k)\} \\[-2pt]
  \FunTab' &= \FunTab \\[-2pt]
  \KontTab' &= \varnothing \quad \text{（本地绑定的续点不逸出）}\\[-2pt]
...
\begin{enumerate}\itemsep0em
    ...
    \item 在续点应用中 $\kappa(v)$；
    \item 由 $\letK\, \kappa(p) = e\, \mathbf{in}\, \ldots$ 绑定
  \end{enumerate}
  这一性质由 CPS IR 的定义保证，因为 \texttt{CRhs} 根本不含续点（也就是说，本 CPS IR 不能返回续点）。有一些读者可能感到失望，
  ...
  表达力是否值得亦是需要探讨并经过调查研究的问题。我们建议，即便必须使用 CPS，也应该考虑 \texttt{Monad} 类型类/接口提供的\emph{内化}（internalized）
...
\begin{remark}[尾调用保证] 指 CPS IR 中的任一函数调用都在尾调用位置。
  \[
    \mathcal{E}\sem{f(p, k)}\cdots\,\text{直接调用}\ f\ \text{而不必压栈}
  \]
...
\begin{align*}
  ...
  \quad\text{\textcircled{\scriptsize 2}}:\text{类型检查}
  \quad\text{\textcircled{\scriptsize 3}}:\text{SysF IR 繁饰、实例解析}\\
  &\text{\textcircled{\scriptsize 4}}:\text{\Lambdair 降级}
  \quad\text{\textcircled{\scriptsize 5}}:\text{CPS 化}
  \quad\text{\textcircled{\scriptsize 6}}:\text{SBCL 目标代码生成}
\end{align*}
...
此外，出现在 CPS IR 中的所有符号，根据 LISP 的语法用 \texttt{||} 保持大小写区别。小节 \ref{sbcl:cav} 曾提到过
因为类型信息已经擦除，我们只能通过按需收集\emph{形状}来缓解歧义问题。形状有定义
\begin{Verbatim}[numbers=left,baselinestretch=1]
inductive Shape where
  | unknown | pair | fn -- function object
...
(define-condition match-failure (error)
  ...
  (:report (lambda (condition stream)
             (format stream
               "No branch can be matched against ~A"
               (discriminant condition)))))
\end{Verbatim}

Common Lisp 针对不同的数据类型有各种各样的等号。
为了减少 LISP 因为动态语言产生的 overhead, 我们依照表 \ref{tab:lisp-eq} 针对
...
\begin{table}[htb]
  \centering
  \caption{LISP 中的等号}\label{tab:lisp-eq}
  \begin{tabular}{@{}ll@{}}
    \toprule
...
当然，因为 \Tigris 编译器看不见也看不懂 FFI 函数的定义，因此其全盘信任用户，后者应
\begin{center}
...
下面以一个稍有复杂的例子说明 FFI 的设计和使用。首先我们利用 FFI 声明引入数个外部函数，
用于 stdin 输入与 stdout 输出，
通过用户自定义算符引入中缀左结合 \texttt{(++)}、前缀 \texttt{(!)} 并将之分别绑定为字符串连接和
即将定义的阶乘函数，即
...
(defforeign "%string-append" (x y) ;; String -> String -> String
  (declare (ignore gamma) (type string x y))
  (funcall k (concatenate 'string x y)))
(defforeign |%read| (_ __) ;; ∀a, Unit -> a
