...
\Tigris 的 REPL 可以通过 \texttt{\#check} 命令打印类型环境表格；CLI 前端则可通过
\texttt{--help} 参数打印帮助信息表格。
\vspace{-.6cm}
\begin{analysis}
  ...
  考虑作为 \texttt{List}（列表）的表头，可将其变换为\emph{“动态”长度}的元组，
  使元组类型\emph{依赖于}表头长，要求元组的类型、长度与表头一致。
...
类型检查器将报错。在此基础上，我们又可定义表格类型 \mintinline{lean4}{TableOf := Array (Row header)}.
...
\end{minted}
其中 \texttt{Align} 和表头形状相同，表示每一列的对齐格式（左对齐、居中、右对齐）；
...
\begin{wrapfigure}{l}{6.5cm}\centering
%\begin{subfigure}{0.35\textwidth}\centering
\includegraphics[scale=0.25]{assets/term2}
%\end{subfigure}%
...
%    \end{minted}
%\end{subfigure}
\caption{表格打印器的输出}\label{pp:comp}
\end{wrapfigure}
...
由此为原点，打印器的实现就无所谓语言而稀松平常了。由于篇幅所限，此处略过。

\section{本章小结}
本章通过 Tigris 实现过程中出现的一个依赖类型编程范例，主要讲解了依赖类型编程的思考方式，
但从实际反响看来，因为其先验理论知识的需求，其流行程度仍然是“足不出象牙塔”的。
因而本章的用意主要在介绍基本的依赖类型知识，以方便下一章的叙述，而对依赖类型不感兴趣的读者也不应将其单纯
看作一个无用的例子。
