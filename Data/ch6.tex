...
与\emph{变量代换}。
\begin{minted}[numbers=left,baselinestretch=1]{haskell}
...
上面略去了一些常量（$\mathsf{Bool}$、$\mathsf{Unit}$）的规则，因为这些规则
与 \Tigris 一致。
...
id x = Lam $ Unbound.bind x $ Var x -- 构建恒等函数的 AST
True = id 'x' === id 'y' -- alpha-equiv: (\x -> x) === (\y -> y)
\end{minted}
...
\begin{figure}\centering
  ...
  \caption{恒等类型的引入规则（introduction rule）在范畴论上的意义\upcite{identity-ty}}\label{fig:id-intro}
\end{figure}
...
譬如此处的 WHNF 消去，可以直接对应着写出具体代码——以 \rref{WHNF-letpair} 为例，我们撷取函数 \texttt{whnf} 的一部分实现：
\begin{minted}[numbers=left,baselinestretch=1]{haskell}
whnf :: Term -> Bi Term
...
上述 WHNF 化工作，或更全面地说，\Tigrisd 的内核运行在 \texttt{Bi} 单子中，定义为
\begin{minted}[numbers=left,baselinestretch=1]{haskell}
...
传统的 ADT 在一个构造子分支内只能定义一个类型，
而索引族之所以称为\emph{族}，是因为其可以定义\emph{一族}（簇）类型。
...
\vspace{-2em}
\begin{analysis}
  存在量化指出，要证明一个命题\emph{存在}，
  \underline{只需举出\emph{一种}满足的情况并证明之即可}——要想编码之，就应当找到一
  代数结构存储其具备的\emph{所有信息}——我们可以通过 dependent coproduct 来编码：
  左右侧分别\emph{封装}\footnote{我们特意使用“封装”一词体现证明论的程序性解释，而非传统逻辑思维。}量化的对象、
  对应的\emph{证明}足矣，即若 $x : A$ 且 $h : B\ x$ 则 $\inmark{x, h}:(x : A) \times B\ x$ 是
