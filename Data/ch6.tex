\chapter{\Tigrisd: a lightweight theorem prover}
\renewottcommands[dd]
...
在实现上，\Tigrisd 只包含四大部分：句法分析器、内核、命令行前端以及一个简单的，可以生成
随机表达式的测试框架。这一部分主要介绍内核与测试框架的设计与实现。
...
必须通过以下最基本形式使用：
\begin{minted}[frame=single]{lean4}
def map {α : Type} {β : Type} {n : Nat}
  (f : α -> β) (as : Vector α n) : Vector β n :=
  match as with
  | Nil         => Nil
...
\begin{itemize}\itemsep0em
  ...
  \item \texttt{show}（命令）：用于指导命令行前端 pretty-print 类型环境
  \item \texttt{rfl}（证明）：为 reflexivity（自反性）的简写，可证明 $a = a$，对任意项 $a$. 可以说这是
  ...
  \item \texttt{exfalso tm}（证明）：即爆炸原理。\texttt{exfalso} 接收一个证伪的项，
    ...
    系统不合理（unsound）。
  \item \texttt{(|>)}（证明）：即等式重写。表达式 $h\triangleright e$ 在左手侧接受一等号类型 $a = b$ 的证明 $h$，
    在右手侧要求 $e = a$，则可以证明 $b$.
...
同时，绝大多数类型论都以\lambdacalc 作为核心代数，因此元相等性也包括 \emph{$\alpha$-等价}。
...
但必须要承认这一设计并不很“函数式”、不很“干净”，存在一些问题。
...
在这里，我们通过 \texttt{EIO} 单子引入——这说明整个内核都是都是\emph{不纯的}，即副作用发生在内核各处——这导致了可维护性问题：
我们无法精确的控制所有的副作用（以及其发生顺序），因为其散落在源代码各处，且程序运行逻辑的复杂性亦加重了这一负担。在 \Tigrisd 的内核中，
...
同理，将等号两侧 WHNF 后我们即可轻易地实现项在内核中的相等判定，由中缀算符 \texttt{(<=>)} 给定，
撷取部分（变量、条件表达式、Let 绑定）下。
\begin{Verbatim}[numbers=left,baselinestretch=1]
...
inductive EvenOddList (α : Type u) : Bool -> Type u where
  ...
  | cons : α -> EvenOddList α b -> EvenOddList α !b' -- b' 是隐式推断的
\end{minted}
